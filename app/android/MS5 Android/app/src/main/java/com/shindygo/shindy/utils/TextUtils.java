...
public class TextUtils {
    ...
    public static String getRemainingStocks(EventInvite event, int gender){
        int m = 0;
        int f = 0;
        int n = 0;
        switch (gender){
            case MALE:
                m = subtractString(event.getMaxMale(), event.getJoinMale());
                if (m>0){
                    f =subtractString(event.getMaxFemale(), event.getJoinFemale());
                    if(f<0){
                        m = m+f;
                    }
                }
            ...
            case FEMALE:
                f =subtractString(event.getMaxFemale(), event.getJoinFemale());
                if (f>0){
                    m =subtractString(event.getMaxFemale(), event.getJoinFemale());
                    if(m<0){
                        f = f+m;
                    }
                }
    ...
    public static int getRemainingStocks(EventInvite event){
        int maxTicket = addString(event.getMaxMale(), event.getMaxFemale());
    ...
    public static String formatTime(EventInvite event, String formatFrom, String formatTo) throws ParseException{
